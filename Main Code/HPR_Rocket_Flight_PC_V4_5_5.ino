...
struct {
  ...
  boolean status_LPS25H = false;  //new
...
void setup(void) {
  ...
  if (settings.testMode) {
    ...
    timeLastBeep = 0UL;
    fltTime.tmClock = 0UL;
    if (!settings.calibrationMode) { settings.testMode = true; }
  }  //end testMode
  ...
  byte unusedBytes = 0;
  for (byte i = 0; i < 4; i++) {
    if ((byte)EEPROM.read(eeprom.baroPressOffset + i) == (byte)255) { unusedBytes++; }
  }
  if (unusedBytes >= 4) {
  ...
  unusedBytes = 0;
  for (byte i = 0; i < 2; i++) {
    if ((byte)EEPROM.read(eeprom.accelBiasX + i) == (byte)255) { unusedBytes++; }
  }
  if (unusedBytes >= 2) {
  ...
  unusedBytes = 0;
  for (byte i = 0; i < 2; i++) {
    if ((byte)EEPROM.read(eeprom.magBiasX + i) == (byte)255) { unusedBytes++; }
  }
  if (unusedBytes >= 2) {
  ...
  if (accel.y0 >= 0) {
    yawY0 = asin(min(1, (float)accel.y0 / (float)g)) * degRad;
  } else {
    yawY0 = asin(max(-1, (float)accel.y0 / (float)g)) * degRad;
  }

  if (accel.x0 >= 0) {
    pitchX0 = asin(min(1, (float)accel.x0 / (float)g)) * degRad;
  } else {
    pitchX0 = asin(max(-1, (float)accel.x0 / (float)g)) * degRad;
  }
  ...
  if (settings.testMode) {
    ...
    Serial.println(((float)offVert) / ((float)10), 2);
  }
  ...
  for (byte i = 0; i < altDigits; i++) {
    if (EEPROM.read(eeprom.maxFltAlt + i) != (byte)255) { initPgm = false; }
  }
...
void loop(void) {
  ...
  if (events.liftoff) {
    ...
    if (abs(accel.z) < accel.ADCmax) {
      ...
      accelNow = (highGsmooth - (float)high1G) * highG.gainZ;
    } else {
      accelNow = (float)(accel.z - g) * accel.gainZ;
    }
    ...
    if (events.timeOut || events.touchdown) {
      ...
      if (!settings.testMode) {
        for (byte i = 0; i < 6; i++) { EEPROM.update(eeprom.maxFltAlt + i, maxAltDigits[i]); }
      }
  ...
  if (!GPSpsm && (events.touchdown || events.timeOut) && micros() - fltTime.touchdown > 5000000UL) {
    GPSpowerSaveMode(settings.testMode);
    GPSpsm = true;
  }
  //Read from serial
  ...
  if (GPS.location.isUpdated() || GPS.altitude.isUpdated()) {
    ...
    if (events.preLiftoff) {
      if (GPS.altitude.meters() != 0) {
        ...
        GPSposn++;
        if (GPSposn >= (byte)(sizeof(GPSavgAlt) / sizeof(GPSavgAlt[0]))) {
          GPSposn = 0;
          GPSbufferFull = true;
        }
...
void parseBeep(long value, byte array[], byte arrayLen) {
  ...
  for (byte i = arrayLen; i >= 1; i--) {
    ...
    if (flag && array[i - 1] == 0) { array[i - 1] = 10; }
  }
}  //end void
